...
async def get_news_list(db: AsyncSession, category_id: int, skip: int = 0, limit: int = 10, keyword: Optional[str] = None):
    # 先尝试从缓存获取新闻列表
    page = skip // limit + 1
    cached_list = await get_cache_news_list(category_id, page, limit, keyword)  # 缓存数据 json
    if cached_list:
    ...
    if news_list:
        ...
        await set_cache_news_list(category_id, page, limit, news_data, keyword=keyword)

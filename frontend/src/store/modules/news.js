...
export const useNewsStore = defineStore('news', {
  ...
  actions: {
    ...
    async getCategories() {
      ...
      try {
        // 调用API获取分类列表
        const response = await axios.get(`${apiConfig.baseURL}/news/categories`);
    ...
    async getNewsList(isRefresh = false) {
      ...
      try {
        ...
        const response = await axios.get(`${apiConfig.baseURL}/news/list`, { params });
    ...
    async getNewsDetail(id) {
      try {
        ...
        const response = await axios.get(`${apiConfig.baseURL}/news/detail?id=${id}`);
